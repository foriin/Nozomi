...
writeXStringSet(unique(shops.a6), "outputs/fasta/a6_short_hoppers_unique.fa")
dir.create("outputs/msa")

# mafft --auto --thread 8 outputs/fasta/a6_short_hoppers_unique.fa > outputs/msa/a6_short_hoppers_uq.mafft.fa

# iqtree2 \
# -s outputs/msa/a6_short_hoppers_uq.mafft.fa \
# -m GTR+G \
# -B 1000 \
# -alrt 1000 \
# -nt AUTO \
# --prefix outputs/msa/a6_short_hoppers_iqtree


tr <- read.tree("outputs/msa/a6_short_hoppers_iqtree.contree")
...
#### Color by K80 ####
aln <- read.dna("outputs/msa/a6_short_hoppers_uq.mafft.fa", format = "fasta")
...
cluster_df <- tibble(
  label = names(cutree(hc, k = 2)),
  hopper_cluster = factor(cutree(hc, k = 2))
)

p_tree_cluster <- ggtree(tr, layout = "unrooted") %<+% cluster_df +
