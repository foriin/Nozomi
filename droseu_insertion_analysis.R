...
gffs <- dir("/Users/artemilin/Work/projects/DrosoTE/popTE/DrosEU/eg_lifted_dm6/", pattern ="*.gff",
            full.names = T)
gna <- sub(".dm6.filteredRepeats.gff", "", basename(gffs))

names(gffs) <- gna

gff_orig <- dir("/Users/artemilin/Work/projects/DrosoTE/popTE/DrosEU/earlgrey/", pattern =".gff",
                full.names = T)
gna2 <- sub("_EarlGrey.filteredRepeats.gff", "", basename(gff_orig))
...
refreps[refreps$type == 'LINE/RTE-X']
...
newins <- lapply(gna, function(x) {
  ...
  data.frame(
    NAME = names(table(grs$NAME)),
    n = as.integer(table(grs$NAME))
  )
})
names(newins) <- gna

df <- bind_rows(newins, .id = 'strain')
dfw <- pivot_wider(df, names_from = strain, values_from = n)
dfwf <- dfw[rowSums(dfw[,-1], na.rm = T) > 10,]
dfwf <- dfwf %>% mutate(across(everything(), ~replace_na(., 0)))
...
ggplot(df_long %>% filter(NAME %in% c("ROO", "HOPPER", "DOC", "COPIA", "POGO", "F-ELEMENT", "HOBO")),
...
print(autoplot(b2_ca1_newdoc, layout = "karyogram")+
  theme_pack_panels()+
  ggtitle("Non-reference isolated Doc insertions in B2-CA1"))
dev.off()

print(autoplot(b2_ca1_newdoc, layout = "karyogram", main = 'heh'))
...
isolatedreps$strainid <- paste(isolatedreps$strain,
                               isolatedreps$ID,
                               sep = "_")
nonrepreps$strainid <- paste(nonrepreps$strain,
                             nonrepreps$ID,
                             sep = "_")
...
# nonref.long <- teorig.te.truec[teorig.te.truec$strainid %in% nonrepreps$strainid]
# dtn.orig <- distanceToNearest(nonref.long)
# isolated.long <- nonref.long[dtn.orig@from][dtn.orig@elementMetadata$distance > 5]
# nonref.long %>% sort() %>% as.data.frame() %>% View

nonref.long <- isolatedreps[isolatedreps$strainid %in% teorig.te.truec$strainid]

###### look at numbers for isolated long ######
newins3 <- lapply(gna, function(x) {
  grs <- nonref.long[nonref.long$strain == x]
  data.frame(
    NAME = names(table(grs$NAME)),
    n = as.integer(table(grs$NAME))
  )
...
df3 <- bind_rows(newins3, .id = 'strain')
dfw3 <- pivot_wider(df3, names_from = strain, values_from = n)
dfwf3 <- dfw3[rowSums(dfw3[,-1], na.rm = T) > 5,]
dfwf3 <- dfwf3 %>% mutate(across(everything(), ~replace_na(., 0)))
...
df_long3 <- dfwf3 %>%
  ...
  mutate(
    value = as.numeric(value),
    genome = factor(genome, levels = setdiff(names(dfwf3), "name"))
  )
...
ggplot(df_long3, aes(x = genome, y = value, group = NAME)) +
  geom_line(alpha = 0.4) +
  # geom_point(size = 1.8) +
  # geom_text_repel(aes(label = NAME), size = 2.5) +
